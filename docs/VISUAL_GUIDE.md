...
- **フレームは 96×96 固定**（キャラ規格`CHAR_FRAME_W/H`と一致）。中央+足元アンカー配置（translate 16,14）。足元原点 (48,84)。
- **2.5頭身チビ**: 大きめ頭＋大きな瞳＋ほお＋髪型＋職業小物＋服の陰影（左上ハイライト/右シェード）。fillRectのみ（クッキリ）。足元影は§6の外部テクスチャ。
- 役割別（`hairStyle`/`prop`/`cap`/`bandana`/`hood`/`beard`で描き分け）:
  商人=まとめ髪+エプロン+コイン袋 / 鍛冶=赤バンダナ+前掛け+ハンマー / ギルド=帽子+本 / 長老=フード+白ひげ+杖 / 村人=かご。
...
- **草の色ムラにタイル散布を使わない**（禁止）。grass2 の大パッチも1タイル散布も、隣接して 2×2+ の塊になり「濃い緑の矩形＝デバッグ領域」に見えた。色ムラはベースタイル内部のモトル＋装飾（草むら/花/小石）だけで出す。矩形クラスターを作らない。
- **建物影は足元・右下のソフト楕円のみ**（`TEX.groundShadow`）。建物全体を覆う暗い矩形は禁止。
